...
@Service
public class R3NotificationListReportGeneratorService implements KyotoProtocolReportTypeService {

    private final DownloadFile downloadFile;

    @Override
    public ReportType reportType() {
        return ReportType.R0022;
    }

    @Override
    public void generateReportData(KyotoProtocolParams params) {
        ConfigLoader cl = ConfigLoader.getConfigLoader();
        NonSEFReport nonSefReportRunner = new NonSEFReportRunner(ReportTypeEnum.R3REG.name());
