...
@Log4j2
public class AOHAAllocationRAReportJdbcMapper
        implements ReportDataMapper<AOHAAllocationRAReportData>, RowMapper<AOHAAllocationRAReportData> {
    ...
    private static final String REPORT_QUERY = "select " +
            ...
            "           sum(case when ae.type = 'NAVAT' then ae.entitlement else 0 end) as navat_entitlement, \n" +
            "           sum(COALESCE(ae.allocated, 0) - COALESCE(ae.returned, 0) - COALESCE(ae.reversed, 0)) as allowances_received, \n" +
    ...
    @Override
    public List<AOHAAllocationRAReportData> mapData(ReportQueryInfoWithMetadata reportQueryInfo) {
        return jdbcTemplate.query(REPORT_QUERY, this);
    }

    @Override
    public AOHAAllocationRAReportData mapRow(ResultSet resultSet, int i) throws SQLException {
        return AOHAAllocationRAReportData.builder()
                .accountHolderName(resultSet.getString("account_holder_name"))
                .aircraftOperatorId(resultSet.getString("aircraft_operator_id"))
                ...
                .navatEntitlement(resultSet.getInt("navat_entitlement"))
                .allowancesReceived(resultSet.getInt("allowances_received"))
