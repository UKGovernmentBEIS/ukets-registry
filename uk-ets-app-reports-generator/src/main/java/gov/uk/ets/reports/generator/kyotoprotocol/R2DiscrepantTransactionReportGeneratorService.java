...
@Service
public class R2DiscrepantTransactionReportGeneratorService implements KyotoProtocolReportTypeService {

    private final DownloadFile downloadFile;

    @Override
    public ReportType reportType() {
        return ReportType.R0021;
    }

    @Override
    public void generateReportData(KyotoProtocolParams params) {
        ConfigLoader cl = ConfigLoader.getConfigLoader();
        NonSEFReport nonSefReportRunner = new NonSEFReportRunner(ReportTypeEnum.R2REG.name());
