...
. Send report request to registry-internal-report-request-topic

[source,bash]
----
kafkacat -P -b localhost:9093 -X sasl.username=client -X sasl.password=password -Xsasl.mechanisms=SCRAM-SHA-512 -Xsecurity.protocol=sasl_plaintext -t report.request.topic -p 0 test-data/r0003.json
----
