...
import * as AccountNotesActions from './account-notes.actions';
...
export class NotesEffect {
  ...
  fetchAcountNotes$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AccountNotesActions.fetchAccountNotes),
      concatLatestFrom(() => this.store.select(isAdmin)),
      filter(([, isAdmin]) => isAdmin),
      switchMap(([payload]) => {
        return this.notesService
          .fetchNotes(payload.accountId, NoteType.ACCOUNT)
          .pipe(
            map((response) =>
              AccountNotesActions.fetchAccountNotesSuccess({ response })
            ),
  ...
  fetchAccountHolderNotes$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AccountActions.loadAccountDetails),
      concatLatestFrom(() => [
        this.store.select(isAdmin),
        this.store.select(selectAccount),
      ]),
      tap(([payload, isAdmin, account]) => console.log()),
      filter(([, isAdmin]) => isAdmin),
      switchMap(([payload, isAdmin, account]) => {
        return this.notesService
          .fetchNotes(String(account.accountHolder.id), NoteType.ACCOUNT_HOLDER)
          .pipe(
            map((response) =>
              AccountNotesActions.fetchAccountHolderNotesSuccess({ response })
            ),
            catchError((httpError) => this.handleHttpError(httpError))
          );
      })
      ...
      return this.actions$.pipe(
        ...
        tap(([action, account]) => {
          this.router.navigate(
            [
              `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.CANCEL_ADD_NOTE}`,
            ],
            {
              skipLocationChange: true,
              queryParams: {
    ...
    () => {
      return this.actions$.pipe(
        ...
        tap(([, account]) =>
          this.router.navigate(
            [
              `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.SELECT_ENTITY}`,
            ],
            {
              skipLocationChange: true,
            }
    ...
    () => {
      return this.actions$.pipe(
        ...
        tap(([, account]) =>
          this.router.navigate(
            [
              `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.ADD_NOTE}`,
            ],
            {
              skipLocationChange: true,
            }
    ...
    () => {
      return this.actions$.pipe(
        ...
        tap(([, account]) =>
          this.router.navigate(
            [
              `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.CHECK_AND_CONFIRM_ADD_NOTE}`,
            ],
            {
              skipLocationChange: true,
            }
    ...
    () => {
      return this.actions$.pipe(
        ...
        tap(([, account]) =>
          this.router.navigate(
            [
              `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.ADD_NOTE_SUCCESS}`,
            ],
            {
              skipLocationChange: true,
            }
  ...
  navigateToDeleteNote$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      tap(([, account]) =>
        this.router.navigate(
          [
            `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.DELETE_NOTE}`,
          ],
          {
            skipLocationChange: true,
          }
    ...
    () => {
      return this.actions$.pipe(
        ...
        tap(([, account]) =>
          this.router.navigate(
            [
              `/account/${account.identifier}/${NotesWizardPathsModel.BASE_PATH}/${NotesWizardPathsModel.DELETE_NOTE_SUCCESS}`,
            ],
            {
              skipLocationChange: true,
            }
