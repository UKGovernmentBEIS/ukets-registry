...
@Component({
  selector: 'app-remove-recovery-phone-form-container',
  template: `
    <app-remove-recovery-phone-form
      [state]="state$ | async"
...
export class RemoveRecoveryPhoneFormContainerComponent {
  ...
  onSubmitRequest(request: RemoveRecoveryPhoneRequest) {
    this.store.dispatch(
      recoveryMethodsActions.REQUEST_REMOVE_RECOVERY_PHONE({ request })
    );
  }
