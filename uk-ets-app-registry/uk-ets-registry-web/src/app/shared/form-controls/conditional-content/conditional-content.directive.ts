  ...
  enableControls(): void {
    const nestedControls: AbstractControl[] = [].concat(
      ...this.childConditionals
        ...
        .map((conditional) =>
          conditional.childControlsAndContainers.map(({ control }) => control)
        )
    );
    ...
    this.childControlsAndContainers
      ...
      .forEach(({ control }) =>
        control.enable({ emitEvent: this.shouldEmitEvent(control) })
      );

    nestedControls
      .filter(
        (control, index) =>
          nestedStatuses[index] === 'DISABLED' && !control.value
      )
      .forEach((control) =>
