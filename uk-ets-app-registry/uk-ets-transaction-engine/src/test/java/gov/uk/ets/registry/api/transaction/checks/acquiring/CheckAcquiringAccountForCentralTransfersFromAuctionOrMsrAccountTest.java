...
class CheckAcquiringAccountForCentralTransfersFromAuctionOrMsrAccountTest {

    @Mock
    TransactionAccountService transactionAccountService;
    @InjectMocks
    private CheckAcquiringAccountForCentralTransfersFromAuctionOrMsrAccount
        checkAcquiringAccountForCentralTransfersFromAuctionOrMsrAccount;
    ...
    void testForAcquiringTotalQuantityAccount() {
        ...
        TransactionSummary transactionSummary = TransactionSummary.transactionSummaryBuilder()
            ...
            .transferringAccountIdentifier(1014L)
        ...
        Mockito.when(transactionAccountService.populateTransferringAccount(any())).thenReturn(
            AccountSummary
                .parse("UK-100-1014-0-27", RegistryAccountType.UK_MARKET_STABILITY_MECHANISM_ACCOUNT, AccountStatus.OPEN)
        );
        ...
        checkAcquiringAccountForCentralTransfersFromAuctionOrMsrAccount.execute(context);
    ...
    @Test
    @DisplayName("Test for not allowed acquiring accounts: New Entrants Reserve Account")
    void testForAcquiringAllocationAccount() {
        ...
        TransactionSummary transactionSummary = TransactionSummary.transactionSummaryBuilder()
            .type(TransactionType.ExternalTransfer)
            .transferringAccountIdentifier(1014L)
            .acquiringAccountIdentifier(1013L)
            .blocks(blocks)
            .build();
    ...
    @Test
    @DisplayName("Test for not allowed acquiring accounts: Allocation Account")
    void testForAcquiringMarketStabilityReserveAccount() {
        ...
        Mockito.when(transactionAccountService.populateAcquiringAccount(any())).thenReturn(
            AccountSummary
                .parse("UK-100-1012-0-37", RegistryAccountType.UK_ALLOCATION_ACCOUNT, AccountStatus.OPEN)
        );
