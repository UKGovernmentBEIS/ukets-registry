...
/**
 * Transactions to acquiring accounts with status ALL TRANSACTIONS RESTRICTED are not allowed.
 */
@Service("check2018")
@BusinessCheckGrouping(groups = BusinessCheckGroup.ACCOUNT)
public class CheckAcquiringAccountHasNotAllTransactionRestrictedStatus extends ParentBusinessCheck {
    ...
    public void execute(BusinessCheckContext context) {
        ...
        if (acquiringAccount != null && AccountStatus.ALL_TRANSACTIONS_RESTRICTED.equals(acquiringAccount.getAccountStatus())) {
            if (context.hasPermission()) {
                addError(context, "Transactions to acquiring accounts with status ALL TRANSACTIONS RESTRICTED are not allowed");
            } else {
    ...
    protected String getITLErrorMessage() {
        return "Acquiring account is not eligible to receive units (2018)";
    }
}
