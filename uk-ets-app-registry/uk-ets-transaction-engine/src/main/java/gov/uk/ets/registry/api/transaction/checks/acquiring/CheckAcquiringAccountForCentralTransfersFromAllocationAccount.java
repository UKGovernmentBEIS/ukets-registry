...
/**
 * The acquiring account must be UK Total Quantity Account or UK New Entrants Reserve when the transferring account
 * is UK Allocation account.
 */
@Service("check2016")
@BusinessCheckGrouping(groups = BusinessCheckGroup.ACCOUNT)
public class CheckAcquiringAccountForCentralTransfersFromAllocationAccount extends ParentBusinessCheck {
    ...
    public void execute(BusinessCheckContext context) {
        boolean isTransferringAccountUkNer = RegistryAccountType.UK_ALLOCATION_ACCOUNT
        ...
        if (isTransferringAccountUkNer &&
            ...
            addError(context, "The acquiring account must be UK Total Quantity Account or UK New Entrants Reserve " +
                "when the transferring account is UK Allocation account");
        }
    }
