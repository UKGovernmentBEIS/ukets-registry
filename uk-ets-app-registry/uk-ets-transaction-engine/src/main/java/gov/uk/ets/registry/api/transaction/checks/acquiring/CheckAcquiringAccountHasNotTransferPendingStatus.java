...
/**
 * Transactions to acquiring accounts with status TRANSFER_PENDING are not allowed.
 */
@Service("check2506")
@BusinessCheckGrouping(groups = BusinessCheckGroup.ACCOUNT)
public class CheckAcquiringAccountHasNotTransferPendingStatus extends ParentBusinessCheck {
    ...
    public void execute(BusinessCheckContext context) {
        ...
        if (acquiringAccount != null && AccountStatus.TRANSFER_PENDING.equals(acquiringAccount.getAccountStatus())) {
            if (context.hasPermission()) {
                addError(context, "Transactions to acquiring accounts with status TRANSFER PENDING are not allowed");
            } else {
    ...
    protected String getITLErrorMessage() {
        return "Acquiring account is not eligible to receive units (2506)";
    }
}
