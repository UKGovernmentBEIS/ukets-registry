...
@TestPropertySource(
        ...
        properties = {
                ...
                "kafka.integration.installation.mets.contacts.enabled=true",
                "logging.level.root=ERROR",
                "account.audit.enabled=false",
        }
)
@EmbeddedKafka(partitions = 1, topics = {
        "installation-registry-contacts-request-topic",
        "registry-installation-contacts-response-topic" })
class InstallationMetsContactsEventConsumerIntegrationTest extends BaseIntegrationTest {
    ...
    void initData(@Autowired TestDataHelper helper) {
        ...
        data2 = dataFactory.nextInstallationAccount();

        helper.createInstallationAccountWithMetsContacts(data1);
        helper.createInstallationAccountWithMetsContacts(data2);
    ...
    @Test
    void shouldConsumeInstallationMetsContactsEventAndAddingNewMetsContactsOverwritingExisting() {

        String correlationId = "corr-aviation-success";

        MetsContactsEvent event = eventHelper.installationMetsContactsEvent(data1.getOperatorIdMetsContacts());
    ...
    @Test
    void shouldConsumeInstallationMetsContactsEventAndNotAddingMetsContactsInCaseOfValidationErrors() {

        String correlationId = "corr-aviation-success";

        MetsContactsEvent event = eventHelper.installationMetsContactsEvent_invalid(data2.getOperatorIdMetsContacts());
