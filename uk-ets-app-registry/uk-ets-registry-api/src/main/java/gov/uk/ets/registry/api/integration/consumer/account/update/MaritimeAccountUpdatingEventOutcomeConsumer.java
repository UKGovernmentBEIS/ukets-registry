...
@KafkaListener(
	    ...
	    topics = {"${kafka.integration.maritime.account.updating.response.topic}"}
	)
@ConditionalOnProperty(name = {"kafka.integration.maritime.account.updating.enabled"}, havingValue = "true")
public class MaritimeAccountUpdatingEventOutcomeConsumer extends AccountUpdatingEventOutcomeConsumer {

	public MaritimeAccountUpdatingEventOutcomeConsumer(GroupNotificationClient groupNotificationClient,
