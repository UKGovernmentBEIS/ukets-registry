...
import gov.uk.ets.registry.api.lock.RegistryLockFacade;
...
public class OAuth2ResourceServerSecurityConfiguration {
    ...
    private RegistryLockFacade registryLockFacade;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests((authorize) -> authorize
              .requestMatchers(HttpMethod.POST,"/api-registry/users.create", 
              "/api-registry/forgot-password.request.link", 
              "/api-registry/user-profile.confirm.email",
              "/api-registry/token-change.request.link",
              "/api-registry/user-profile.emergency.otp.request",
              "/api-registry/user-profile.emergency.otp.task",
              "/api-registry/forgot-password.validate.token",
              "/api-registry/forgot-password.reset.password",
              "/api-registry/user-profile.emergency.password-otp.request",
    ...
    public CommandLineRunner keycloakAuthorizationDataLoader(ObjectMapper mapper) {
        return new CommandLineRunner() {
            ...
            final Header header = new BasicHeader(HttpHeaders.USER_AGENT, "registry-web-initial-security-config");
            ...
            public void run(String... args) {
                try {
                    if (registryLockFacade.execute(apiAuthzConfig, r -> authorizationsLoader(mapper).doLoad(config, r))) {
                        log.info("Authorizations have been updated.");
                    } else {
                        log.info("Authorizations were already updated.");
                    }
                } catch (Exception e) {
    ...
    @Autowired
    public void setRegistryLockFacade(RegistryLockFacade registryLockFacade) {
        this.registryLockFacade = registryLockFacade;
    }
}
