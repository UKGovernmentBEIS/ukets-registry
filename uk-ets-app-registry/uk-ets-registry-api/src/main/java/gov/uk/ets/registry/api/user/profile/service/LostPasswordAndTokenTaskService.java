...
public class LostPasswordAndTokenTaskService implements TaskTypeService<TokenTaskDetailsDTO> {
    ...
    public Set<RequestType> appliesFor() {
        return Set.of(RequestType.LOST_PASSWORD_AND_TOKEN);
    }
    ...
    public TaskCompleteResponse complete(TokenTaskDetailsDTO taskDTO, TaskOutcome taskOutcome, String comment) {
        ...
        if (TaskOutcome.APPROVED.equals(taskOutcome)) {
            ...
            // TODO what happens if one of the requests bellow fails?
            try {
                serviceAccountAuthorizationService
                    .addRequiredActionToUser(user.getIamIdentifier(), CONFIGURE_UPDATE_PASSWORD_REQUIRED_ACTION);
                serviceAccountAuthorizationService
                    .addRequiredActionToUser(user.getIamIdentifier(), CONFIGURE_OTP_REQUIRED_ACTION);
                serviceAccountAuthorizationService
                    .triggerKeycloakExecuteActionsEmail(user.getIamIdentifier(), CONFIGURE_UPDATE_PASSWORD_REQUIRED_ACTION);
            } catch (Exception e) {
                throw new IllegalArgumentException("Task could not be completed",e);
            }
