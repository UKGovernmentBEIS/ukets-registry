...
@KafkaListener(
	    ...
	    topics = {"${kafka.integration.aviation.account.opening.response.topic}"}
	)
@ConditionalOnProperty(name = {"kafka.integration.aviation.account.opening.enabled"}, havingValue = "true")
public class AviationAccountOpeningEventOutcomeConsumer extends AccountOpeningEventOutcomeConsumer {

    public AviationAccountOpeningEventOutcomeConsumer(GroupNotificationClient groupNotificationClient,
    ...
    /**
     * Handles aviation account creation outcome.
     * @param outcome
     * @param headers
     */
    @KafkaHandler
	@Transactional
    public void consumeAviation(AccountOpeningEventOutcome outcome, @Headers Map<String, Object> headers) {
