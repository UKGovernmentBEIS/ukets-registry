...
public class InstallationRegulatorNoticeEventConsumer extends RegulatorNoticeEventConsumer {

    public InstallationRegulatorNoticeEventConsumer(RegulatorNoticeEventService service, KafkaTemplate<String, RegulatorNoticeEventOutcome> kafkaTemplate, EventConsumerTopicsUtil topicsUtil) {
        super(service, kafkaTemplate, topicsUtil);
    }

    @KafkaListener(
            containerFactory = "installationRegulatorNoticeConsumerFactory",
            topics = "${kafka.integration.installation.regulator.notice.request.topic}"
    )
    @Transactional
    public void installationRegulatorNoticeConsumerFactory(RegulatorNoticeEvent event,
