...
TEMP_FILE_NAME="$BASE_FILE_PATH"/migrations/migration_0037-hashing-iterations-password-policy-argon/uk-ets-realm-updated-policy-temp.json
...
if
  ...
  "$BASE_FILE_PATH"/migrations/migration_0037-hashing-iterations-password-policy-argon/uk-ets-realm-updated-policy.json >${TEMP_FILE_NAME}
then
...
if [[ $HTTP_CODE -eq 204 ]]; then
  echo "Fixed password policy and hashed iterations to 'uk-ets-password-policy'"
else
  echo "Failed to fix password policy and hashed iterations to 'uk-ets-password-policy(url)', exiting..."
  exit 2
fi
